...
while True:
    ...
    if flag:
        ...
        putText(img,"Make sure only your face with mask is being detected",(20,30),FONT_HERSHEY_SIMPLEX,0.65,(255,255,255),4,lineType=LINE_AA)
        putText(img,"Make sure only your face with mask is being detected",(20,30),FONT_HERSHEY_SIMPLEX,0.65,(0,0,0),2,lineType=LINE_AA)
        ...
        for x,y,w,h in faces:
            ...
            putText(img,"Face Detected! "+ str(len(data))+"/200",(x,y),FONT_HERSHEY_SIMPLEX,1,(244,250,250),2)
            face=img[y:y+h,x:x+w,:]
            face=resize(face,(50,50))
            print(len(data))
            if len(data) < 200:
                data.append(face)
        imshow('Training with Mask',img)
        if waitKey(2)==27 or len(data) >=200: # pressing esc will force quit so it is not recommended as we need 200 images
            print('Done Training with mask')
            break
cap.release()
destroyAllWindows()

save('with_mask.npy',data)
